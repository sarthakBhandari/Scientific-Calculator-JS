...
function calculator(button){
    if(button.type=="operator"){
        ...
        //FIXING THE POWER AND FACTORIAL ISSUE
        /*SEACH FOR POWER AND FACTORIAL FUNCTIONS*/
        let POWER_SEARCH_RESULT = search(data.formula, POWER);
        let FACTORIAL_SEARCH_RESULT = search(data.formula, FACTORIAL);
        

        /*GET POWER BASE AND REPLACE WITH THE RIGHT BASE*/
        ...
        /*GET FACROIAL NUMBER AND REPLACE WITH THE RIGHT FORMULA*/
        const NUMBERS = factorialNumberGetter(data.formula, FACTORIAL_SEARCH_RESULT);
        console.log(NUMBERS);
        NUMBERS.forEach(object =>{
            formula_str = formula_str.replace(object.toReplace, object.replacement);
        })
        console.log(formula_str);
        ...
        updateOutputResult(result);
        return;//we dont want to change top-output
    }
...
function factorialNumberGetter(formula, FACTORIAL_SEARCH_RESULT){
    ...
    FACTORIAL_SEARCH_RESULT.forEach( factorial_index =>{
        let number = [];
        next_input_index = factorial_index + 1;
        if(formula[next_input_index]==FACTORIAL){
